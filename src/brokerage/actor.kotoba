(ns brokerage.actor
  "SecuritiesBrokerageActor — the ISCO-08 3311 community brokerage actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one brokerage operation request
  ...
  The unconditional invariant: the SecuritiesBrokerageAdvisor can never
  directly commit a record the SecuritiesBrokerageGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [brokerage.advisor :as advisor]
            [brokerage.governor :as governor]
            [brokerage.store :as store]))

(defn build-graph
  "Build a compiled SecuritiesBrokerageActor graph. `store` implements
  `brokerage.store/Store`. `advisor` implements
  `brokerage.advisor/Advisor` (defaults to `mock-advisor`).
